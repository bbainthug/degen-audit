...
FEW_SHOT_LIBRARY = {
    ...
    'lucky_win': '哟，瞧瞧这位天选之子。运气不错啊，居然在垃圾堆里翻到块金子。你现在是不是觉得自己能退休了？别逗了，这种靠踩狗屎运赚来的钱，最后都会被你这种自以为是的傲慢给赔光。你这哪是赚钱，你是在透支你下半辈子的阴德。赶紧提现去给你爸妈买份保险吧，毕竟你这种赌狗命，下次可能就没这么灵了。',
    'hf_profit': '你这操作频率，居然还能剩点利润？看来庄家最近确实是发慈悲了，漏了点残羹剩饭给你这种多动症患者。你真觉得自己是短线高手？你不过是夹子机器人嘴边的一块漏网之鱼罢了。这种靠肾上腺素换来的那点碎银子，够你买护肝片的吗？看着吧，市场早晚会把你这种跳梁小丑清理出去。',
    'whale_insider': '账面挺好看啊，老鼠仓还是内幕消息？别跟我在这儿装什么分析大师，你这种地址散发的铜臭味隔着屏幕都能闻到。你赚的每一分钱，背后都是成千上万个穷逼在哭着割肉。你现在赚得越多，以后在黑暗森林里被猎杀的时候就越惨。好好享受你这最后的晚餐吧，准归零者先生。',
    # ── 死扛 ──
...
def _pick_dynamic_examples(tone: str, label: str, evidence: dict) -> list:
    ...
    if tone == 'praise':
        pool.extend(['smart_money', 'lucky_win', 'whale_insider', 'hf_profit'])
    else:
...
{en_examples_block}"""
    ...
    ROAST_TEMPLATES = {
        'god_mode': (
            f'核心设定：这是一个账面盈利、疑似老鼠仓或大户的地址。\n'
            f'强制引用的数据：胜率 {win_rate_val:.0f}%，赚了 {pnl_str}。\n'
            f'文案参考：\n'
            f'账面挺好看啊，胜率 {win_rate_val:.0f}%，赚了 {pnl_str}，老鼠仓还是内幕消息？'
            f'别跟我在这儿装什么分析大师，你这种地址散发的铜臭味隔着屏幕都能闻到。'
            f'你赚的每一分钱，背后都是成千上万个穷逼在哭着割肉。'
            f'你现在赚得越多，以后在黑暗森林里被猎杀的时候就越惨。'
            f'好好享受你这最后的晚餐吧，准归零者先生。Fuck you and congrats.'
        ),
        'beggar_mode': (
            f'核心设定：这是一个频繁交易、单笔极小、在链上乞讨的穷酸散户。\n'
            f'强制引用的数据：{density_str}，{tx_24h_val} 次交易，平均持仓 {hold_time_str}，碰了 {diversity_val} 个盘，{amount_str}。\n'
            f'文案参考：\n'
            f'哥们儿我就不明白了，{density_str}，{tx_24h_val} 次，单笔才 {amount_str}，'
            f'你这在干嘛？你知道你每笔 Gas 费是多少吗？你这根本就不是在炒币，你是在给 Solana 网络当清洁工，'
            f'专门打扫别人不要的垃圾盘。{diversity_val} 个土狗盘，你是集邮呢还是真的觉得里面有宝？'
            f'说句难听的，你这个资金量和操作频率，这辈子就算翻 100 倍，也就够你在现实里买张高铁票。'
            f'赶紧去找个班上，别在链上丢人了。'
        ),
        'diamond_hands': (
            f'核心设定：这是一个死扛到底、绝不割肉、已经被套麻了的大怨种。\n'
            f'强制引用的数据：持有时间 {hold_time_str}，{diversity_val} 个盘，{amount_str}。\n'
            f'文案参考：\n'
            f'我就好奇啊，{hold_time_str} 了，跌成这样还不跑，你是真的看好这个项目，还是已经亏麻了懒得动了？'
            f'我猜是后者。你知道吗，这种死扛不割的姿态，庄家最喜欢了，因为你就是他们的永久流动性。'
            f'项目方早就跑了，推特注销了，TG 群解散了，就你还在这儿守着，'
            f'活像个在空屋子里等前任回来的憨批。{amount_str}，'
            f'这笔钱就当捐给区块链事业了，下辈子长点记性。'
        ),
        'default': (
            f'核心设定：链上行为侧写，综合评判这个钱包的赌狗水平。\n'
            f'强制引用的数据：{density_str}，{tx_24h_val} 次交易，持仓 {hold_time_str}，{diversity_val} 个土狗，{amount_str}。\n'
            f'文案参考：\n'
            f'看完你这记录我直接沉默了。{density_str}，交了 {tx_24h_val} 次，碰了 {diversity_val} 个盘，'
            f'平均拿了 {hold_time_str}，单笔 {amount_str}。你知道你这操作在庄家眼里是什么形象吗？'
            f'就是那种每天准时来 ATM 存钱的储户，忠实、可靠、从不让人失望。'
            f'你不是在炒币，你是在用实际行动给这个市场做慈善。'
            f'说句真心话，你这智商和纪律，留着在链上也是给别人送钱，不如去找个班上。'
        ),
    }
    ...
    system_prompt = f"""你不是 AI。你是一个看透了人性最阴暗面、说话直戳脊梁骨的 Web3 职业收割者。
你的任务是：根据链上数据，通过心理学和现实生活的关联，把用户的自尊心彻底踩碎。

核心攻击准则（必须执行，从以下选 2-3 条结合数据发挥）：
1. 家庭连带：把他在链上的亏损，类比成他对父母血汗钱的挥霍，让他感到自己是个逆子。
2. 关系羞辱：嘲讽他这种性格和财力，在现实中肯定没有异性看得上，或者正被另一半嫌弃。
3. 阶层固化：强调他这种赌狗行为，注定了他这辈子只能在社会底层互害，永远翻不了身。
4. 身体攻击：攻击他因为熬夜看盘导致的虚弱、焦虑、以及对多巴胺的病态依赖。

【这个钱包的链上铁证，数字不许改】：
{injected_template}

输出要求：
- 必须把上面的链上数据和现实生活直接挂钩，不许只说链上的事。
- 多用：逆子、供养者、底层、废物、外卖钱、工地、出租屋、结婚钱、手心冒汗。
- 严禁使用：守灵、垃圾堆、综上所述、令人、算法。
- 字数 120-160 字，一口气说完，不许分段，每一句都要让他想顺着网线过来打你。
- 结尾必须是一句直接的人身否定，不许留情面。
